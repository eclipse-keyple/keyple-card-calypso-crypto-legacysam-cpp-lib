...
const std::map<int, const std::shared_ptr<StatusProperties>>
    CommandDigestUpdate::STATUS_TABLE = [] {
        ...
        m.insert(
            {{0x6700,
             ...
             {0x6A80,
              std::make_shared<StatusProperties>(
                  std::string("Incorrect value in the incoming data: session")
                      + " in Rev.3.2 mode with encryption/decryption active "
                        "and not"
                      + " enough data (less than 5 bytes for and odd "
                        "occurrence or "
                      + "less than 2 bytes for an even occurrence)",
...
CommandDigestUpdate::CommandDigestUpdate(
...
CommandDigestUpdate::getProcessedData() const
{
    return mProcessedData;
}

void
CommandDigestUpdate::finalizeRequest()
{
    /* Nothing to do */
}

bool
CommandDigestUpdate::isControlSamRequiredToFinalizeRequest() const
{
    return false;
}

void
CommandDigestUpdate::parseResponse(
    const std::shared_ptr<ApduResponseApi> apduResponse)
{
    setResponseAndCheckStatus(apduResponse);

    mProcessedData = apduResponse->getDataOut();
}

const std::map<int, const std::shared_ptr<StatusProperties>>&
CommandDigestUpdate::getStatusTable() const
{
    return STATUS_TABLE;
}
