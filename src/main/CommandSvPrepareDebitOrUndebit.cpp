...
const std::map<int, const std::shared_ptr<StatusProperties>>
    CommandSvPrepareDebitOrUndebit::STATUS_TABLE = [] {
...
CommandSvPrepareDebitOrUndebit::CommandSvPrepareDebitOrUndebit(
    std::shared_ptr<CommandContextDto> context,
    std::shared_ptr<SvCommandSecurityDataApi> data)
: Command(
      data->getSvCommandPartialRequest()[0] == 0xBA
...
, mData(data)
{
    ...
    /* header(4) + SvDebit data (12) = 16 bytes */
    std::vector<std::uint8_t> dataIn(16 + data->getSvGetResponse().size());
...
void
CommandSvPrepareDebitOrUndebit::finalizeRequest()
{
    /* Nothing to do */
}

bool
CommandSvPrepareDebitOrUndebit::isControlSamRequiredToFinalizeRequest() const
{
    return false;
}

void
CommandSvPrepareDebitOrUndebit::parseResponse(
...
const std::map<int, const std::shared_ptr<StatusProperties>>&
CommandSvPrepareDebitOrUndebit::getStatusTable() const
{
    return STATUS_TABLE;
}
